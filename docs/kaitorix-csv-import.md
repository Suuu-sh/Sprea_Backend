...
CSV全件を毎日そのままD1へ展開することはしません。25,000商品・37店舗のスナップショットを約75,000件の見積もりに分解すると、D1無料枠の読み書きを再び圧迫するためです。原本はR2に保持し、Actionsで高額かつ本人確認できる候補だけを抽出します。D1には商品ごとの最高買取価格を投影し、上位店舗と店舗数は属性として残します。候補インポートは同じJAN・同じ取得時刻の再実行を no-op にするため、途中で失敗しても同じ日のActionsを再実行できます。販売APIの探索は候補ごとのプロバイダー状態をキューとして一度だけマテリアライズし、期限到来分をインデックスで取得します。結果なしは24時間、プロバイダーエラーは最長7日のバックオフとし、日次CSVでも既存キューの期限を保持します。再構築には排他ロックと15分の失敗バックオフを設け、手動操作とCronの重複で同じ候補を何度も書き込まないようにします。失敗後も次回から再開します。CSVの更新がない5分間隔の実行では、買取見積もりや候補全体を再スキャンしません。

探索キューが空のときは実行履歴を書き込まず、Dashboard・Analyticsなどの集計APIは短時間キャッシュします。これにより、結果の更新を止めずにアイドル時のD1読み書きを抑えます。

本番の全SQLはR2の共通安全予算（UTC日で読取り200万行・書込み6万行）で予約・精算されます。予算不足時は次のSQLを実行せず、未完了処理を翌日に持ち越します。CSV取込のoffsetから再開可能です。日次候補の自動削除は行いません。詳細はREADMEのD1 safety budgetを参照。
