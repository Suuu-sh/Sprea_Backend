...
5. Actions側でCSVを展開し、JANが正確で新品・未使用、アクセサリー以外、商品/買取価格が5,000円以上の行だけを候補化（買取価格上位5,000件まで。`KAITORIX_MAX_CANDIDATES`で調整可能）
6. 候補を500件ずつ `/admin/kaitorix-csv/import-candidates` へ送り、D1には商品ごとの最高買取価格と上位店舗情報だけを保存
7. 最終バッチが探索キューをdirtyにし、Workerの5分Cronが一度だけ待機行を作成（CSV取込直後に探索を重複起動しない）
