...
const quote = (overrides: Record<string, unknown> = {}) => ({jan: undefined, modelNumber: undefined, condition: "new" as const, ...overrides});

describe("buyback product matching", () => {
  it("matches an exact JAN with maximum confidence", () => expect(matchProduct(quote({jan:"4900000000001"}),products)).toEqual({matched:true,productId:"jan-product",confidence:1,reason:"jan_exact"}));
