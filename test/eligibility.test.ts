...
const retail = (overrides: Partial<RetailListing> = {}): RetailListing => ({
  ...
  stockStatus: "in_stock", purchasable: true, productUrl: "https://example.test/item",
...
const buyback = (overrides: Partial<BuybackQuote> = {}): BuybackQuote => ({
  id: "buyback-1", productId:"product-1", provider: "fixture", sourceType: "manual", productName: "Camera X",
...
describe("isEligibleRetailListing", () => {
  it.each(["in_stock", "low_stock"] satisfies StockStatus[])("accepts new, purchasable %s listings", stockStatus => {
    expect(isEligibleRetailListing(retail({stockStatus}),new Date("2026-08-29T01:00:00Z"))).toBe(true);
  });
...
describe("isEligibleBuybackQuote", () => {
  it("accepts new quotes that are accepting", () => expect(isEligibleBuybackQuote(buyback(),new Date("2026-08-29T01:00:00Z"))).toBe(true));
