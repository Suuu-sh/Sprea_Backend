...
Cloudflare Cronは探索キューを5分ごと、評価・Collectorを6時間ごと（UTC）に実行します。同じ時刻の再実行では価格・取引・評価を重複させません。候補が空の探索実行ではD1へ履歴を書き込みません。探索で結果が出なかったペアは24時間、プロバイダーエラーは最長7日間再試行を待機します。CSV更新でも既存キューの期限はリセットしません。Dashboard・Analyticsなどの集計APIは短時間キャッシュし、Evaluatorは各期限以後で最初の売却価格を使います。日次CSV取得はGitHub Actionsで実行します。モデル学習の全DBエクスポートは手動のみです。
...
本番のHTTP/cronは `src/d1-budget.ts` を共通で通ります。R2の条件付き書込でSQL実行前に読み取り・書き込みを予約し、成功時のみD1の `meta.rows_read/rows_written` で精算します。失敗・強制終了時は予約を残します。

- UTC日単位で読み取り200万行・書き込み6万行の安全予算。次のSQLの予約が入らなければ429 `d1_budget_paused` で待機。UTC 00:00に予算を切替。
- R2障害・不明なSQL・自動の一括削除は安全側に停止。D1テーブルの追加は不要。
- CSV原本はR2に全件保持。D1投影は最大5,000商品。同じCSVは途中のバッチから再開し、同一行は再書込しません。
- 候補・キューの書込みを100行ずつに分割。候補の日時だけの変化では再書込せず、日次の一括削除・再作成はしません。
- `GET /api/research/resource-budget` はR2のみを読み、予約を含む当日の利用量を返します。アカウント全体の実使用量ではありません。
- 無料枠到達前に保留することを優先するため、1日で全件が終わる保証はありません。未処理のキューは翌日継続します。
- 外部のDB利用・手動のWrangler操作・別アプリはこの予算の対象外。残り4万書込み/300万読込みは予備ですが、アカウント全体を無条件に保証するものではありません。

2026-09-21 ローカルD1/R2検証: 5,000商品の翌日CSV更新と288回の探索（販売APIは該当なしのモック）で133,522読込み / 47,963書込み。初回構築は40,612読込み / 47,403書込みで次のバッチを予約できず保留。実データの結果件数・履歴サイズによって変わるため、この値を本番の保証値にはしません。
