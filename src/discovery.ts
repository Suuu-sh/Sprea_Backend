...
type RakutenCandidate=Pick<Candidate,"model_number"|"product_name"> & {jan?:string|null};
const rakutenTextEncoder=new TextEncoder();
function trimRakutenKeyword(value:string,maxBytes=128):string{let output="";for(const character of value){const next=output+character;if(rakutenTextEncoder.encode(next).length>maxBytes)break;output=next;}return output;}
function validRakutenKeyword(value:string):boolean{const keyword=value.trim();if(!keyword||rakutenTextEncoder.encode(keyword).length>128)return false;return keyword.split(/\s+/).every(token=>token.length>=2||/[^\x00-\x7F]/u.test(token));}
function cleanRakutenKeyword(value:string):string{return trimRakutenKeyword(value.normalize("NFKC").replace(/[\[【].*?[\]】]/g," ").replace(/新品|未開封|送料無料|買取価格|SIMフリー/gi," ").replace(/\s+/g," ").trim());}
export function rakutenDiscoveryQuery(candidate:RakutenCandidate):string{const model=cleanRakutenKeyword(candidate.model_number??"");if(validRakutenKeyword(model))return model;const embedded=candidate.product_name.match(/\b(?:[A-Z0-9]{4,}J\/A|CFI[- ]?\d{4}[A-Z]\d{2})\b/i)?.[0];if(embedded&&validRakutenKeyword(embedded))return embedded;const name=cleanRakutenKeyword(candidate.product_name);if(validRakutenKeyword(name))return name;const jan=typeof candidate.jan==="string"&&/^\d{8,14}$/.test(candidate.jan)?candidate.jan:"";return validRakutenKeyword(jan)?jan:"";}
export function rakutenDiscoveryQueries(candidate:RakutenCandidate):string[]{const primary=rakutenDiscoveryQuery(candidate),capacity=primary.match(/\b\d+(?:GB|TB)\b/i)?.[0]??"",compact=cleanRakutenKeyword(primary.replace(/\b(iPhone|iPad)\s+(\d+)/i,"$1$2").replace(/\s+(Pro|Max|Air)\b/gi,"$1").replace(/\s+\d+(?:GB|TB)\b.*$/i,"")),fallback=cleanRakutenKeyword([compact,capacity].filter(Boolean).join(" "));return[...new Set([primary,fallback,compact].filter(validRakutenKeyword))];}
...
export async function runProductDiscovery(env:DiscoveryEnv,trigger="manual",limit=10,at=new Date()){
 ...
 // pending and will be picked up on the next invocation. Manual clicks also
 // have a short cooldown so repeated clicks cannot spend the same write budget
 // again immediately after a successful batch.
