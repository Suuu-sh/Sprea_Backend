...
export async function runProductDiscovery(env:DiscoveryEnv,trigger="manual",limit=10,at=new Date()){
 ...
 const foundCandidates=new Set<number>(),searchedCandidates=new Set<number>(),providerStats:Record<string,{searched:number;found:number;listings:number;profitable:number;threshold:number;failures:number;lastError:string}>={},lastRequest=new Map<string,number>();
 for(const name of names)providerStats[name]={searched:0,found:0,listings:0,profitable:0,threshold:0,failures:0,lastError:""};
...
export async function discoveryFunnel(db:D1Database){
 ...
 const providers=latestRuns.map(row=>({provider:String(row.provider),found:Number(row.found_count??0),listings:Number(row.listing_count??0),profitable:Number(row.profitable_count??0),threshold:Number(row.threshold_count??0),averageProfitGap:0,lastError:String(row.last_error??"")}));
...
export async function discoveryQueueStatus(db:D1Database){
 ...
 const providers=run?(await db.prepare("SELECT provider,searched_count,found_count,listing_count,profitable_count,threshold_count,failure_count,last_error FROM product_discovery_provider_runs WHERE run_id=? ORDER BY provider").bind(run.id).all<any>()).results:[];
