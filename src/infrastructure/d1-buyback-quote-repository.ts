...
export class D1BuybackQuoteRepository implements BuybackQuoteRepository {
  ...
  async upsertLatest(quote: BuybackQuote): Promise<void> {
    ...
    await this.db.prepare(`INSERT INTO buyback_quotes
      (id,product_id,provider,source_type,external_id,product_name,jan,model_number,brand,category,condition,attributes_json,price,shipping_fee,fee,buyback_status,product_url,fetched_at,last_seen_at,created_at,updated_at)
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ...
      last_seen_at=excluded.last_seen_at,updated_at=excluded.updated_at`).bind(
      ...
      quote.fetchedAt, quote.lastSeenAt, now, now,
    ).run();
  }
