...
from train import metrics, train
...
def test_train_rejects_empty_dataset(tmp_path: Path):
    ...
    with sqlite3.connect(db) as conn:
        conn.executescript("""CREATE TABLE opportunities(id INTEGER,detected_at TEXT,buy_cost_yen INTEGER,market_profit_yen INTEGER,resolver_confidence REAL);
        CREATE TABLE evaluations(opportunity_id INTEGER,horizon_hours INTEGER,realized_profit_yen INTEGER);""")
    try:
